...
import styles from "./community-repo-page.module.css";
...
function AvailableDatasets({
  ...
  const ucKind = kind[0]?.toUpperCase() + kind.slice(1);

  return (
    <div>
      <h3 className={styles.centered}>Available {kind}s</h3>
      {datasets.length === 0 ? (
...
function _createDatasetListing({
  ...
  list: CommunityDataset[];
  user: string;
}): DatasetType[] {
  if (!list) return [];

  // Note that the `request` always includes @branch, irregardless of URL.
  return list.map((d: CommunityDataset): DatasetType => {
    return {
