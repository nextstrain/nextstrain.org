 ...
 *   when `github.com/:user/:repo` is not a public repo -> display only
...
export default function CommunityPageRouter({
  ...
  // these flags control whether we show the "main" content or the
  // "repo-specific" content, and whether we display an error bar or
  // not. "main" versus "repo-specific" are exclusive of each other;
  // an error may be shown by itself or with main/repo content,
  // depending.
  ...
  // these will only be used if we're showing the error banner, but setting them here doesn't hurt anything.
  const errorContents = <p>Here is the community page instead.</p>;
  const errorTitle = `The community repository, dataset, or narrative "${errorPath}" doesn't exist.`;

  return (
    <>
      {showError && (
        <ErrorMessage title={errorTitle} contents={errorContents} />
      )}
