 ...
 * contains path elements past `/nextclade`, and, if so, returns a
...
export default function ErrorBanner(): React.ReactElement | null {
  ...
  if (params && params["nextclade"]) {
    // n.b., I don't think `params["nextclade"]` is ever going to be
    // anything other than a list, but let's make the type checker
    // happy…
    const path =
      typeof params["nextclade"] === "string"
        ? params["nextclade"]
        : params["nextclade"].join("/");

    const resourceType = path.startsWith("narratives")
      ? "narrative"
      : "dataset";

    const title = `The Nextclade ${resourceType} "nextstrain.org/nextclade/${path}" doesn't exist.`;
    const contents = <p>Here is the Nextclade reference trees page instead.</p>;
