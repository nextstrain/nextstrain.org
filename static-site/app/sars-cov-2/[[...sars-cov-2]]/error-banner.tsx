 ...
 * contains path elements past `/sars-cov-2`, and, if so, returns a
...
export default function ErrorBanner(): React.ReactElement | null {
  ...
  if (params && params["sars-cov-2"]) {
    // n.b., I don't think `params["sars-cov-2"]` is ever going to be
    // anything other than a list, but let's make the type checker
    // happy…
    const path =
      typeof params["sars-cov-2"] === "string"
        ? params["sars-cov-2"]
        : params["sars-cov-2"].join("/");

    const resourceType = path.startsWith("narratives")
      ? "narrative"
      : "dataset";

    const title = `The ${resourceType} "nextstrain.org/sars-cov-2/${path}" doesn't exist.`;
    const contents = (
      <p>
