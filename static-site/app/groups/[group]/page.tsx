...
export default function IndividualGroupPage({
  ...
  const [narratives, setNarratives] = useState<Resource[]>([]);
  ...
  useEffect(() => {
    ...
    async function getGroupInfo(): Promise<void> {
      try {
        ...
        setDatasets(availableData.datasets.map(d => convertToResource(d, "dataset")));
        setNarratives(availableData.narratives.map(n => convertToResource(n, "narrative")));
