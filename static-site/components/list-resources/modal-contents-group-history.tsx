...
import { FilterOption, Group, PathVersionsForGroup } from "./types";
...
function _filterHistory(history: GroupHistory, filters: string[]): GroupHistory {
...
function _changelog(pathVersions: PathVersionsForGroup): GroupHistory {
  const dates = _orderedDates(pathVersions);
  const indexes = Object.fromEntries(dates.map((d, i) => [d, i]));
  const history: GroupHistory = dates.map((d) => [d, {}]);
