...
/**
 * TODO XXX - this entire file is just Modal.jsx with some changes as I didn't
 * want to be constrained and implement the conditional logic just yet.
 */
...
export const ResourceModalFiles = ({data, dismissModal}) => {  
  ...
  useEffect(() => {
    if (!ref || !data) return;
    _draw(ref, data, setSelectedDates)
  ...
  return (
    <div ref={scrollRef}>
      ...
      <ModalContainer onClick={(e) => {e.stopPropagation()}}>
        ...
        <p/>
        <Table fileUrls={data.fileUrls} dates={selectedDates}/>
        ...
        <div style={{fontStyle: 'italic'}}>
          ...
          <br/>
          <div onClick={() => setSelectedDates(data.dates)} style={{textDecoration: 'underline', cursor: 'pointer'}}>
            Click here to reset the table
          </div>
        </div>
      </ModalContainer>
...
function _draw(ref, data, setSelectedDates) {
    ...
    .join("circle")
      ...
      .on("mouseover", function(e, d) {
        ...
        selectedVersionGroup.select(".message")
          ...
          .text(`Intermediate files from ${prettyDate(d.data.version)} (click to filter)`)        
      })
      ...
      .on("click", function(e, d) {
        setSelectedDates([d.data.version])
      })
    ...
    .append("rect")
      ...
      .on("mousemove", function(e) {
        ...
        selectedVersionGroup.select(".message")
          ...
          .style("opacity", 1)
          .style("text-anchor", textR ? "start" : "end")
          .text(`On ${prettyDates[0]} the latest intermediate files were from ${prettyDates[1]} (click to filter)`)
      })
      .on("mouseleave", function() {
        beeswarm.join(
          (enter) => {}, /* eslint-disable-line */
      ...
      .on("click", function(e) {
        ...
        setSelectedDates([datum.data.version])
      })
...
const Table = ({fileUrls, dates}) => {
  // fileUrls = date->name->url
  const filenames = [... new Set(Object.values(fileUrls).map((o) => Object.keys(o)).flat())].sort();
  const datesReversed = dates.slice().reverse()
      ...
      <tbody>
        {filenames.map((filename) => (
          <tr key={filename}>
            ...
            {datesReversed.map((date) => (
              <td key={date}>
                {fileUrls?.[date]?.[filename] ? (
                  <a href={fileUrls[date][filename]}  target="_blank" rel="noreferrer">link</a>
                ) : (
                  <span>{`❌`}</span>
                )}
              </td>
