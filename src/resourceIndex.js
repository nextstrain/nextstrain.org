...
class ListResources {
  ...
  get data() {
    ...
    if (this.resourceType === 'dataset') {
      result[this.resourceType][this.sourceId].pathVersions = Object.fromEntries(_resources.map(_formatDatasets));
    } else if (this.resourceType === 'intermediate') {
      if (this.groupHistory) {
        result[this.resourceType][this.sourceId].pathVersions = Object.fromEntries(
          _resources
            .filter(([name,]) => name.split('/')[0]===this.groupHistory)
            .map(_formatIntermediates)
        );
      } else {
        result[this.resourceType][this.sourceId].latestVersions = Object.fromEntries(_resources
          .map(_formatIntermediates)
