...
const listResourcesJson = async (req, res) => {
  ...
  const sourceId = req.params.page;
  const resources = new ListResources([req.params.page], [resourceType]);
  const data = {
    WARNING: `This API is intended for internal use only. The API, including the address, may change at any point without notice.`,
    ...resources.data
  }
  utils.verbose(`[LIST RESOURCES] Sending JSON for source "${sourceId}" resource type "${resourceType}"`);
